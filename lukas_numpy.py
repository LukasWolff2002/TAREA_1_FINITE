...
class Desplazamientos:
    ...
    def rc_vector (self):
        print("\nReacciones")
        rc = self.kcf @ self.uf + self.kcc @ self.uc
        print(rc)
        return rc
#1000 * A

#Extraer con indices
#np._ix(filas, columnas, M)
...
n2 = Node(1, np.array([2, 3]), np.array([0, 0, 0]), np.array([3, -1, 0]))
n3 = Node(2, np.array([6, 3]), np.array([0, 0, 0]), np.array([0, -1, 0]))
